...
<template>
	<tr :data-id="token.id"
		...
		<td class="token-name">
			...
			<span v-if="wiping" class="wiping-warning">({{ t('nmcsettings', 'Marked for remote wipe') }})</span>
		</td>
		<td>
		...
		<td class="more">
			<NcActions v-if="!token.current"
				:title="t('nmcsettings', 'Device settings')"
				:aria-label="t('nmcsettings', 'Device settings')"
				...
				<template v-if="token.canDelete">
					<template v-if="token.type !== 2">
						<NcActionButton icon="icon-delete"
							...
							<!-- TODO: add text/longtext with some description -->
							{{ t('nmcsettings', 'Revoke') }}
						</NcActionButton>
						<NcActionButton icon="icon-delete"
							@click.stop.prevent="wipe">
							{{ t('nmcsettings', 'Wipe session') }}
						</NcActionButton>
					</template>
					<NcActionButton v-else-if="token.type === 2"
						icon="icon-delete"
						:title="t('nmcsettings', 'Revoke')"
						@click.stop.prevent="revoke">
						{{ t('nmcsettings', 'Revoking this token might prevent the wiping of your device if it has not started the wipe yet.') }}
					</NcActionButton>
				</template>
			</NcActions>
		</td>
...
const nameMap = {
	ie: t('nmcsettings', 'Internet Explorer'),
	edge: t('nmcsettings', 'Edge'),
	firefox: t('nmcsettings', 'Firefox'),
	chrome: t('nmcsettings', 'Google Chrome'),
	safari: t('nmcsettings', 'Safari'),
	androidChrome: t('nmcsettings', 'Google Chrome for Android'),
	iphone: t('nmcsettings', 'iPhone'),
	ipad: t('nmcsettings', 'iPad'),
	iosClient: t('nmcsettings', '{productName} iOS app', { productName: window.oc_defaults.productName }),
	androidClient: t('nmcsettings', '{productName} Android app', { productName: window.oc_defaults.productName }),
	iosTalkClient: t('nmcsettings', '{productName} Talk for iOS', { productName: window.oc_defaults.productName }),
	androidTalkClient: t('nmcsettings', '{productName} Talk for Android', { productName: window.oc_defaults.productName }),
	davx5: 'DAVx5',
	webPirate: 'WebPirate',
	...
	computed: {
		...
		iconName() {
			...
			if (matches) {
				...
				this.token.name = t('nmcsettings', 'Sync client - {os}', {
					os: matches[1],
					version: matches[2],
				})
				icon = 'icon-desktop'
			...
			if (this.token.current) {
				name = t('nmcsettings', 'This session')
			}
